package top.headfirst.funding.filter;
...
import top.headfirst.funding.constant.AccessPassResources;
...
@Component
public class FundingAccessFilter extends ZuulFilter {
    @Override
    ...
    public Object run() throws ZuulException {
        ...
        Object loginMember = session.getAttribute(FundingConstant.ATTR_NAME_LOGIN_MEMBER);
        // 4.判断loginMember是否为空
        if (loginMember == null){
            // 5.从requestContext对象中获取Response对象
            HttpServletResponse response = currentContext.getResponse();
            // 6.将提示消息存入Session域
            session.setAttribute(FundingConstant.ATTR_NAME_MESSAGE,FundingConstant.MESSAGE_ACCESS_FORBIDEN);
            try {
